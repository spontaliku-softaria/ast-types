...
module.exports = function () {
    ...
    Tp.toString = function () {
        ...
        if (isString.check(this.name)) {
            name = this.name;
        } else if (isFunction.check(this.name)) {
            name = this.name.call(this) + "";
        } else {
            name = this.name + " type";
        }

        if (builtInTypes[name] || namedTypes[name] || defCache[name]) {
            return name;
        } else {
            return '"' + name + '"';
        }
    ...
    Dp.finalize = function () {
        ...
        if (!self.finalized) {
            ...
            var builder = builders[getBuilderName(self.typeName)];
            if (builder) {
                builder.type = self.typeName;
                builder.params = self.buildParams.map(function(param) {
                    var field = allFields[param];
                    if (!field) {
                        console.warn("Missing field for build param '" + param + "' in type '" + self.typeName + "'");
                        return {
                            name: param,
                            type: ''
                        }
                    ...
                    return {
                        name: param,
                        type: typeof field.type.name === "function" ? field.type.name() : field.type.name,
                        optional: !!field.defaultFn
                    }
                });
            }
        }
